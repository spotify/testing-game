    ...
    for name in files:
        ...
        if fileextension == '.m' or fileextension == '.mm':
            ...
            try:
                with open(absfile) as sourcefile:
                    source = sourcefile.read()
                    p = subprocess.Popen(['git', 'blame', '-e', absfile], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    out, err = p.communicate()
...
for t in sorted_list:
    ...
    print "%(i)d. %(n)s, %(t)d (%(p).2f%%)" % { 'i': sorted_list.index(t) + 1, 'n' : name_mapping[t[0]], 't' : t[1], 'p' : percentage }
